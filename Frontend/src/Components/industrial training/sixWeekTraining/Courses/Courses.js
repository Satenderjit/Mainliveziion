...
const courseData = [
  {
    ...
    subcategories: ['Web Development', 'PHP', 'Front-End Frameworks', 'Back-End Architecture'],
  },
  {
    ...
    subcategories: ['Mobile Application Development', 'Web Designing', 'UI/UX Principles', 'Responsive Design'],
  },
  {
    ...
    subcategories: ['Artificial Intelligence', 'Machine Learning', 'Deep Learning', 'Neural Networks'],
  },
  {
...
const Courses = () => {
  ...
  const currentImageUrl = activeDomain ? activeDomain.imageUrl : courseData[0]?.imageUrl;
      ...
      <div className={styles.leftSection}>
        <div className={styles.textContainer}>
            <h1 className={styles.title}>
              Shape Your Future with <span className={styles.highlight}>Expert-Led Courses</span>
            </h1>
            <p className={styles.description}>
              Unlock your potential with our comprehensive curriculum. We provide hands-on training in the most in-demand technology fields to prepare you for the jobs of tomorrow.
            </p>
            <button className={styles.exploreButton}>Explore All Courses</button>
        </div>

        <div key={currentImageUrl} className={styles.imageCard}>
          <img src={currentImageUrl} alt={activeDomain ? activeDomain.name : 'Technology Course'} className={styles.cardImage} />
        </div>
      </div>
        ...
        <div className={styles.industryList}>
          {courseData.map((domain) => (
            <div key={domain.name} className={styles.industryItem}>
              <div className={`${styles.industryHeader} ${openDomain === domain.name ? styles.open : ''}`} onClick={() => toggleDomain(domain.name)}>
              ...
              <div className={`${styles.subcategoriesWrapper} ${openDomain === domain.name ? styles.open : ''}`}>
                <div className={styles.subcategories}>
                    {domain.subcategories.map((sub, index) => (
                      <span key={index} className={styles.subcategoryTag} style={{ animationDelay: `${index * 50}ms` }}>
                        {sub}
                      </span>
                    ))}
                </div>
